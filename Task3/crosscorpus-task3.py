...
# Configuration
#enter iemocap model for ravdess dataset or use ravdess model for iemocap dataset
# Configuration will be set by command line arguments
iemocap_emotions = ["ang", "hap", "neu", "sad"]
ravdess_emotions = ["neutral", "happy", "sad", "angry"]
...
def seed_everything(seed):
    ...
    # set to false for AWMC, since it use train mode
    torch.use_deterministic_algorithms(True)
    
    #torch.use_deterministic_algorithms(True, warn_only=True)

def is_scripted(example):
    return "script" in example["audio"]["path"]

def is_improvisation(example):
    return "impro" in example["audio"]["path"]
...
def load_and_prepare_dataset(dataset_name):
    if dataset_name == "iemocap":
        ...
        dataset1 = load_dataset("xbgoose/ravdess", split="train")
        from datasets import concatenate_datasets
        ravdess_emotions = ["neutral", "happy", "sad", "angry"]
        train_dataset = dataset1.remove_columns([
            'modality', 'vocal_channel', 'emotional_intensity', 
            'statement', 'repetition'
        ])
        train_dataset = train_dataset.filter(lambda x: x["emotion"] in ravdess_emotions)
        train_dataset = train_dataset.cast_column("emotion", ClassLabel(num_classes=4, names=ravdess_emotions))
        #prepare for test data -- iemocap
        #Match the iemocap emotions to the ravdess emotions
        model_order = ["neutral", "happy", "sad", "angry"]
        dataset_order = ["ang", "hap", "neu", "sad"]
        alias = {"ang":"angry", "hap":"happy", "neu":"neutral", "sad":"sad"}
        # dataset idx -> model idx
        dataset2model = {i: model_order.index(alias[name]) for i, name in enumerate(dataset_order)}
        # -> {0:3, 1:1, 2:0, 3:2}

        def remap_label(ex):
            ex["label"] = dataset2model[ex["label"]]
            return ex
        print("before remap_label", dataset["session1"][0])
        # Apply to every split you’ll use
        dataset = dataset.map(remap_label)
        print("after remap_label", dataset["session1"][0])
        ...
        dataset1 = load_dataset("Zahra99/IEMOCAP_Audio")
        from datasets import concatenate_datasets
        #map iemocap dataset to the model label
        #Match the iemocap emotions to the ravdess emotions
        model_order = ["neutral", "happy", "sad", "angry"]
        dataset_order = ["ang", "hap", "neu", "sad"]
        alias = {"ang":"angry", "hap":"happy", "neu":"neutral", "sad":"sad"}
        ...
        def remap_label(ex):
            ex["label"] = dataset2model[ex["label"]]
            return ex
        print("before remap_label", dataset1["session1"][0])
        # Apply to every split you’ll use
        dataset1 = dataset1.map(remap_label)
        print("after remap_label", dataset1["session1"][0])
        ...
        ravdess_emotions = ["neutral", "happy", "sad", "angry"]
        test_dataset = dataset.remove_columns([
            'modality', 'vocal_channel', 'emotional_intensity', 
            'statement', 'repetition'
        ])
        test_dataset = test_dataset.filter(lambda x: x["emotion"] in ravdess_emotions)
        print("before cast_column", test_dataset[0])
        test_dataset = test_dataset.cast_column("emotion", ClassLabel(num_classes=4, names=ravdess_emotions))
        print("after cast_column", test_dataset[0])
        test_dataset = test_dataset.cast_column("audio", Audio(sampling_rate=16000))
        dataset1 = DatasetDict({
            "train": train_dataset, 
            "test": test_dataset
...
def load_pretrained_model(model_checkpoint):
...
def run_tta_evaluation(model, processor, test_all_dataset, tta_method="tent", source_dataset=None, test_dataset_name="iemocap"):
    ...
    if tta_method == "source":
        ...
        encoded_test = encoded_test.rename_column("label", "labels") #used for dataset iemocap
        #encoded_test = encoded_test.rename_column("emotion", "labels") #used for dataset ravdess
        ...
        tta_model = lame.LAME(net, knn=10, sigma=1.0, affinity='kNN', force_symmetry=True)
        ...
        tta_model = dsuta.DSUTA(fast_model, slow_model, fast_optimizer, slow_optimizer, update_freq=5, memory_size=5, adaptation_steps=10, temperature=1.5, entropy_weight=0.3, device='cuda', class_num=4)
        ...
        #anchor_optimizer = torch.optim.AdamW(param_anchor, lr=2e-5, betas=(0.9, 0.999), weight_decay=0., foreach=False)
        anchor_optimizer = torch.optim.AdamW(param_anchor, lr=1e-5, betas=(0.9, 0.999), weight_decay=0., foreach=False)
        #leader_optimizer = torch.optim.AdamW(param_leader, lr=2e-5, betas=(0.9, 0.999), weight_decay=0., foreach=False)
        leader_optimizer = torch.optim.AdamW(param_leader, lr=1e-5, betas=(0.9, 0.999), weight_decay=0., foreach=False)
        #system_optimizer = torch.optim.AdamW(param_system, lr=2e-5, betas=(0.9, 0.999), weight_decay=0., foreach=False)
        ...
        raise ValueError(f"Unsupported TTA method: {tta_method}. Supported: ['source', 'tent', 'sar', 'suta', 'cotta', 'foa' ,'cea', 't3a', 'lame', 'eata', 'dsuta', 'awmc']")
    ...
    if test_dataset_name == "iemocap":
        encoded_dataset = encoded_dataset.rename_column("label", "labels")
    elif test_dataset_name == "ravdess":
        encoded_dataset = encoded_dataset.rename_column("emotion", "labels")
    encoded_dataset.set_format(type="torch", columns=["input_values", "labels"])
    
    # Create DataLoader
    dataloader = DataLoader(encoded_dataset, batch_size=args.batch_size, shuffle=False)  # Use batch size from command line args
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="IEMOCAP/RAVDESS Emotion Recognition with Test-Time Adaptation")
    parser.add_argument(
        "--tta_method", 
        type=str, 
        default="tent",
        choices=["source", "tent", "sar", "suta", "cotta", "foa" ,'cea', 't3a', 'lame', 'eata', 'dsuta', 'awmc'],
    ...
    parser.add_argument(
        ...
        type=str, 
        default="iemocap",
        choices=["iemocap", "ravdess"],
        help="Test dataset name: 'iemocap' or 'ravdess'"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    if args.test_dataset_name == "iemocap":
        model_checkpoint = "hongdage/back_iemocap"
    elif args.test_dataset_name == "ravdess":
        model_checkpoint = "hongdage/wav2vec2-base-ravdess22"

    # Load and prepare dataset
    dataset0 = load_and_prepare_dataset(args.test_dataset_name)
    dataset1, test_all_dataset = dataset0, dataset0["test"]
    
    # Load pre-trained model and processor
    processor, model = load_pretrained_model(model_checkpoint)
    ...
    if args.tta_method == "foa":
        ...
        if args.test_dataset_name == "iemocap":
            encoded_source = encoded_source.rename_column("emotion", "labels")
        elif args.test_dataset_name == "ravdess":
    ...
    tta_results = run_tta_evaluation(model, processor, test_all_dataset, tta_method=args.tta_method, source_dataset=source_dataset, test_dataset_name=args.test_dataset_name)
