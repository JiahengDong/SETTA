#!/usr/bin/env python3
"""
RAVDESS Emotion Recognition with Test-Time Adaptation (TTA)
...
def load_and_prepare_dataset():
    ...
    dataset000 = dataset.remove_columns([
    ...
    print("Available actors:", set(dataset000['actor']))
    print("Available genders:", set(dataset000['gender']))
    
    # Resample to 16kHz and set up emotion labels
...
def create_train_test_splits(dataset0):
    """Create train/test splits based on actor IDs"""
    print("Creating train/test splits...")
    
    # Filter datasets based on 'actor' values
    train_dataset = dataset0.filter(lambda x: 1 <= x['actor'] <= 16).remove_columns(['actor', 'gender'])
    validation_dataset = dataset0.filter(lambda x: 17<=x['actor'] <= 20).remove_columns(['actor', 'gender'])
    
    # Note: There's a bug in the original notebook - this line should be fixed
    test_all_dataset = dataset0.filter(lambda x: 21 <= x['actor'] <= 24).remove_columns(['actor', 'gender'])
    ...
    return dataset1, test_all_dataset
...
def run_tta_evaluation(model, processor, test_all_dataset, tta_method="tent", source_dataset=None):
    ...
    if tta_method == "source":
        ...
        tta_model = sar.SAR(net, optimizer, margin_e0=0.4*math.log(8), reset_constant_em=0.2)
        ...
        encoded_test = encoded_test.rename_column("emotion", "labels")
        ...
        optimizer = torch.optim.AdamW(params, lr=1e-5, betas=(0.9, 0.999), weight_decay=0., foreach=False)
        tta_model = eata.EATA(sub_net, optimizer, fisher_alpha=2000.0, steps=1, episodic=False, e_margin=0.4*math.log(8), d_margin=0.04, fishers=fishers)
        ...
        tta_model = t3a.T3A(net, num_classes=8, filter_K=64)
        ...
        tta_model = dsuta.DSUTA(fast_model, slow_model, fast_optimizer, slow_optimizer, update_freq=20, memory_size=20, adaptation_steps=10, temperature=1.5, entropy_weight=0.3, device='cuda', class_num=8)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="RAVDESS Emotion Recognition with Test-Time Adaptation")
    parser.add_argument(
        "--tta_method", 
        type=str, 
        default="tent",
    ...
    print(f"Starting RAVDESS emotion recognition with TTA method: {args.tta_method}")
    
    # Load and prepare dataset
    dataset0 = load_and_prepare_dataset()
    dataset1, test_all_dataset = create_train_test_splits(dataset0)
    ...
    if len(labels) != model.config.num_labels:
        ...
        print("Solutions:")
        print("1. Use a model trained on RAVDESS")
