...
# Configuration
model_checkpoint = "LincolnD/wav2vec2-base-finetuned-iemocap-acted-to-natural-emotion"
# Configuration
...
def load_and_prepare_dataset():
    ...
    train_scripted = dataset.filter(is_scripted)
    train_improvisation = dataset.filter(is_improvisation)

    from datasets import concatenate_datasets
    train_dataset = concatenate_datasets([
    train_scripted["session1"],
    train_scripted["session2"],
    train_scripted["session3"],
    train_scripted["session4"],
    train_scripted["session5"]
    ])

    test_dataset = concatenate_datasets([
        train_improvisation["session1"],
        train_improvisation["session2"],
        train_improvisation["session3"],
        train_improvisation["session4"],
        train_improvisation["session5"]
    ])
...
def run_tta_evaluation(model, processor, test_all_dataset, tta_method="tent", source_dataset=None):
    ...
    # Create DataLoader
    dataloader = DataLoader(encoded_dataset, batch_size=args.batch_size, shuffle=False)
...
if __name__ == "__main__":
    ...
    print(f"Starting IEMOCAP emotion recognition with TTA method: {args.tta_method} and batch size: {args.batch_size}")
